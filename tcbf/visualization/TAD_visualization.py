...
class Triangle(object):
    def __init__(self, url, chrom, start, end, correct='weight', figsize=(7, 3.5)):
...
class Multiple_Cool(object):
    def __init__(self,cool_files,reference,chrom,start,end,TAD_boundary_group,sample_order = None,):

        self.TAD_group = TAD_boundary_group
        names = [(i.split(".")[0],i) for i in cool_files]
        self._cool = {k:cooler.Cooler(v) for k,v in names}
        self.reference = self._cool.get(reference)
        if not self.reference:
            raise FileNotFoundError("没有提供对应的参考基因组的cool文件。")
        self.binsize = self.reference.binsize

    def find_synteny_region(self,reference,chrom,start,end):
        pass
